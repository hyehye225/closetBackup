...
export default class CodyAddComponent extends Component {
  ...
  saveCody(e) {
    ...
    if (
      (this.state.outerId || this.state.topId) == null ||
      this.state.bottomId == null
    ) {
      alert('아우터/상의와 하의를 선택하세요');
    } else {
      alert('코디가 등록되었습니다!');
      const testCody = {
        codyId: null,
      ...
      testCody.codyId= 'cody-' + new Date().getTime();
      testCody.userId = this.state.userId;
  ...
  setOuter = () => {
    ...
    this.setState({ outerId: cid }, () => {
      ...
      if (this.state.outerId == null) {
        ...
        this.setState({ openOuterModal: false });
        ClosetService.getAOuter(this.state.userId, this.state.outerId)
          .then((res) => {
            
            const data = res.data;

            this.setState((this.state.outer = data));
  ...
  setTop = () => {
    ...
    this.setState({ topId: cid }, () => {
      if (this.state.topId == null) {
        ...
        this.setState({ openTopModal: false });
        ClosetService.getATop(this.state.userId, this.state.topId)
          .then((res) => {
            const data = res.data;

            this.setState((this.state.top = data));
  ...
  setBottom = () => {
    ...
    this.setState({ bottomId: cid }, () => {
      if (this.state.bottomId == null) {
        ...
        this.setState({ openBottomModal: false });
        ClosetService.getABottom(this.state.userId, this.state.bottomId)
          .then((res) => {
            const data = res.data;

            this.setState((this.state.bottom = data));
  ...
  render() {
    ...
    // console.log("o "+this.state.outer);
    // console.log("t "+this.state.top);
    // console.log("b "+this.state.bottom);
    ...
    return (
      ...
      >
        {this.state.submitted ? (
          ...
          <div>
            <h3 style={{ fontFamily: 'Tenada' }}>코디 등록</h3>
